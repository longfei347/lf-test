	...
	var fetchContent = function() {
		...
		if (parseInt(BROWSER.ie) != 6) {
			loadingst = setTimeout(function() {
				showDialog('', 'info', '<img src="' + DIALOGIMGDIR + '/loading.gif"> 请稍候...')
			}, 3);
		}
	};
...
(function($) {
	$.fn.microshop_like = function(options) {
		...
		function submit_like() {
			var item = $(this);
			$.getJSON("index.php?act=like&op=like_save", {
				type: settings.type,
				like_id: item.attr("like_id")
			}, function(json) {
				if (json.result == "true") {
					if (settings.count_target == '') {
						item.find("em").microshop_count({
							type: "+"
						});
					} else {
						settings.count_target.microshop_count({
							type: "+"
						});
					}
				}
